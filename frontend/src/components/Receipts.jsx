...
function Receipts() {
    ...
    const fieldsStep1 = [
        { name: "checkNumber", label: "Receipt Number" },
        {
            ...
            label: "Employee",
            type: "fk",
            fetchUrl: "http://127.0.0.1:5174/employee-by-ID",
        ...
        },
        {
            ...
            label: "Card",
            type: "fk",
            fetchUrl: "http://127.0.0.1:5174/customers-card",
    ...
    const fieldsStep2 = [
        {
            ...
            label: "UPC",
            type: "fk",
            fetchUrl: "http://127.0.0.1:5174/products-in-store",
            optionValue: "UPC",
    ...
    const addProduct = () => {
        ...
        const totalSum = updatedProducts.reduce((sum, p) => sum + parseFloat(p.selling_price), 0);
        const totalVat = updatedProducts.reduce((sum, p) => sum + parseFloat(p.vat), 0);
            ...
            for (const field of allFields) {
                if (field.type === 'fk' && field.fetchUrl) {
                    try {
                        ...
                        const parsed = JSON.parse(json.body).data;
    ...
    return (
        <div className="receipts-container">
            <SearchAndBack />
            <button className='addNewReceipt' onClick={() => setShowModal(true)}>Add</button>
                ...
                <div className="receipt-modal-overlay" onClick={closeModal}>
                    <div className="receipt-modal-content" onClick={e => e.stopPropagation()}>
                        <div className="modal-left">
                            ...
                            <p><strong>Sum:</strong> {formData.sumTotal}</p>
                        ...
                        <div className="modal-right">
                            <div className="receipt-modal-header">
                                ...
                                <button type="button" onClick={closeModal} className="close-button">×</button>
                            </div>
                            ...
                            {(step === 1 ? fieldsStep1 : fieldsStep2).map(field => (
                                <div key={field.name}>
                                    {field.type === 'fk' ? (
                                        <select
                                            name={field.name}
                                            value={step === 1 ? formData[field.name] : productData[field.name]}
                            ...
                            <div className="modal-actions">
                                {step === 1 ? (
                                    ...
                                    <>
                                        <button className="save-button" onClick={addProduct}>Add Product</button>
                                        <button
                                            className="save-button"
                                            onClick={() => {
